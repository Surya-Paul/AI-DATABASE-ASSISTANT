...
4. In the Vercel **Environment Variables** setup (before clicking Deploy), add:
   - `ANTHROPIC_API_KEY` set to your real API key.
...
4. In the Render dashboard, go to your service → **Environment** tab →
   Set `ANTHROPIC_API_KEY` to your real API key.
...
3. Build and run:
   ...
   docker run -d -p 80:8000 -e ANTHROPIC_API_KEY=sk-ant-your-key ai-sql-assistant
   ```
...
- **API Key**: For all options, set the `ANTHROPIC_API_KEY` environment
  variable to your real Anthropic API key. Without it, the app runs in
