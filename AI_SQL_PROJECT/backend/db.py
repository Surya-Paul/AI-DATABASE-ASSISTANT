...
def execute_query(sql: str, confirmed: bool = False) -> dict:
    ...
    # Strip any trailing semicolons which break subqueries
    sql = sql.strip()
    if sql.endswith(';'):
        sql = sql[:-1].strip()
...
def load_dataframe_to_db(df: pd.DataFrame, table_name: str) -> dict:
    """Loads a pandas DataFrame into a new SQLite table, replacing if it exists."""
    try:
        # Sanitize column names to remove spaces/special characters
        # This prevents the AI from needing to use quotes in SQL queries
        df.columns = [re.sub(r'[^a-zA-Z0-9_]', '_', str(col)).strip('_') for col in df.columns]
        
        df.to_sql(table_name, engine, if_exists='replace', index=False)
        return {"success": True, "message": f"Table '{table_name}' loaded successfully."}
    except Exception as e:
